...
class ogameBot():
    ...
    def calculate_production(self, coord_mask = "%", check = False):
        ...
        for id in planet_list:
            ...
            if not x:
                print 'No reports found for planet {}. We need at least two.'.format(self.db.get_planet_coord(id))
                continue
            elif len(x) < 2:
                print 'Need another report for planet {} to calculate production.'.format(self.db.get_planet_coord(id))
                continue
